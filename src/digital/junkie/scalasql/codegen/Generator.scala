  ...
  def impl(
    ...
    def generate(
      ...
      SchemaReader.forConnection(conn).map { reader =>
        ...
        val tableColumns = tables.map(t => (t, reader.readColumns(conn, schema, t)))
        ...
        val imports: List[Stat] = List(
          ...
          Option.when(usedTypes.contains("LocalDate"))(q"import java.time.LocalDate"),
          Option.when(usedTypes.contains("LocalDateTime"))(q"import java.time.LocalDateTime")
        ).flatten

        val classStats: List[Stat] = tableColumns.flatMap { case (tableName, columns) =>
          ...
          val params: List[Term.Param] = columns.map { col =>
            ...
            val tpe: Type = if (col.nullable) t"Option[$baseType]" else baseType
            Term.Param(Nil, Term.Name(columnNaming(col.name)), Some(tpe), None)
          }
          List(
