...
trait SchemaReader {
  ...
  def readColumns(conn: Connection, schema: String, table: String): List[ColumnInfo]
...
object SchemaReader {
  ...
  object SQLite extends SchemaReader {
    ...
    private def sqliteTypeToScala(typeName: String): String = {
      ...
      else if (t.contains("CHAR") || t.contains("TEXT") || t.contains("CLOB")) "String"
      else if (t.contains("DATETIME") || t.contains("TIMESTAMP")) "LocalDateTime"
      else if (t.contains("DATE")) "LocalDate"
      else if (t.contains("BOOL")) "Boolean"
      else if (t.contains("REAL") || t.contains("DOUBLE") || t.contains("FLOAT")) "Double"
